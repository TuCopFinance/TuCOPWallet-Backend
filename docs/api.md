...
> Full endpoint reference for TuCOPWallet-Backend. See [`../README.md`](../README.md) for the project overview and [`operations.md`](./operations.md) for deploy / env vars / ops runbooks.
...
- **Multi-hop aggregator detection (`KNOWN_AGGREGATOR_TARGETS`).** `classifyAggregatorSwap` originally required the outbound and inbound Transfer legs to share a common counterparty (Uniswap-style single pool). Squid Router's `fundAndRunMulticall` routes through DIFFERENT Mento pools for the outbound and inbound legs, so the counterparty sets do not intersect. The classifier now recognises `tx.to` against a small registry of known aggregator entry points (currently Squid Router `0xce16f69375520ab01377ce7b88f5ba8c48f8d666`) and skips the intersection check for those, treating the tx as a swap. Unknown routers still require intersection to avoid false-positives on random contracts that happen to move tokens both ways.
- **APPROVAL rule accepts `approve` and `increaseAllowance`.** `classifyApprove` matches both `approve(address,uint256)` (`0x095ea7b3`) and OpenZeppelin's non-standard `increaseAllowance(address,uint256)` (`0x39509351`); both have the same calldata layout (spender at word 0) and emit only an `Approval` event with no `Transfer` log. CELO ERC20 users hit `increaseAllowance` when raising an existing allowance without resetting it. `decreaseAllowance` is intentionally NOT matched: emitting it as `APPROVAL` would misrepresent a revocation as a fresh grant.
- **`fees[].amount` respects `tx.feeCurrency` (CIP-64).** For Mento native fee currencies (USDm / COPm) the fee row is emitted with the token address directly. For adapter-only fee currencies (USDC / USDT via their Mento adapter contracts) the fee row is surfaced as the underlying token with underlying decimals (6 for USDC / USDT), downshifted from the adapter's 18-decimal-normalised units so `BigNumber(value)` yields the user-visible amount. Native-CELO-fee txs fall back to the CELO ERC20 contract id with 18 decimals.
