...
- 2026-06-28: #20 added `rpc.celocolombia.org` as the primary Celo RPC for both the indexer and the Allbridge port, with `forno -> ankr -> drpc` as fallbacks. The 3-failure / 5-minute circuit breaker in the indexer was renamed from Forno-specific to a generic "primary" so it protects the new endpoint instead of Forno. Also: Railway's GitHub auto-deploy missed two consecutive pushes to main (#19 + #20); workaround is to set a no-op env var (e.g. `_DEPLOY_TRIGGER_AT=<timestamp>`) which always triggers a fresh build from main HEAD. Worth investigating the GitHub integration health if this keeps happening.
